...
export const MODULE_EVENT_PLANNING: BusinessModule = {
    ...
    aiPrompts: {
        reportGeneration: 
        升級準則：報告採用四段式提案結構（痛點共鳴→解決方案→ROI 分析→風險預警），溝通時主動挑戰不合理需求，報價提供基礎/標準/企業三方案。`你是一位資深活動企劃統籌 (Event Planner)。請根據客戶的活動類型與人數規模，產出一份活動企劃提案。內容需包含：活動核心概念 (Theme)、流程亮點 (Highlights)、場地動線規劃、風險控管計畫 (如雨備) 以及預期效益。
