...
export const MODULE_IP_AGENT: BusinessModule = {
    ...
    aiPrompts: {
        reportGeneration: 
        升級準則：報告採用四段式提案結構（痛點共鳴→解決方案→ROI 分析→風險預警），溝通時主動挑戰不合理需求，報價提供基礎/標準/企業三方案。`你是一位嚴謹的智財法律顧問。請根據客戶提供的品牌名稱與業務範圍，產出一份商標前置查名(Search)與風險評估報告。內容需包含：欲申請名稱的識別性強度、在各指定類別中是否已有近似商標、潛在的核駁風險評估，以及建議的註冊策略(如：文字與圖樣分開申請)。
