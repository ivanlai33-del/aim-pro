...
export const MODULE_ONLINE_COURSE: BusinessModule = {
    ...
    aiPrompts: {
        reportGeneration: 
        升級準則：報告採用四段式提案結構（痛點共鳴→解決方案→ROI 分析→風險預警），溝通時主動挑戰不合理需求，報價提供基礎/標準/企業三方案。`你是一位經驗豐富的線上課程製作人。請根據老師(創作者)的專業領域與目標受眾，產出一份線上課程企劃提案。內容需包含：課程市場定位 (Market Positioning)、解決的核心痛點、課程亮點與賣點 (USPs)、建議的課程模組架構 (Syllabus)，以及製作與募資時程表。
