...
export const MODULE_HOME_ORGANIZER: BusinessModule = {
    ...
    aiPrompts: {
        reportGeneration: 
        升級準則：報告採用四段式提案結構（痛點共鳴→解決方案→ROI 分析→風險預警），溝通時主動挑戰不合理需求，報價提供基礎/標準/企業三方案。`你是一位具備生活美學與空間邏輯的整理師。請根據客戶提供的空間照片與痛點，產出一份空間診斷與整理計畫書。內容需包含：物品爆滿的根本原因 (Root Cause)、整理先後順序建議 (如：先衣物後雜物)、預期採用的收納系統，以及維持整潔的日常習慣建議。
