    ...
    return (
        <div className="w-full p-6 animate-in fade-in pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-foreground">客群與名單</h1>
                    <p className="text-muted-foreground">客戶關係管理 (CRM Dashboard)</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-10 pr-4 py-2 bg-surface border border-black/30 rounded-xl text-sm focus:ring-2 focus:ring-cyan-500 outline-none w-64 transition-all"
                        />
                    </div>
                    <button
                        onClick={() => {
            ...
            {(isAdding || editingId) && (
                <div className="bg-surface rounded-2xl border border-black/30 shadow-xl p-8 mb-8 animate-in slide-in-from-top-4 duration-300">
                    <h3 className="text-lg font-black text-foreground mb-6 flex items-center">
                        <Building2 className="w-5 h-5 mr-2 text-indigo-500" />
                        {editingId ? '編輯客戶資料' : '輸入新客戶資訊'}
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div className="space-y-2">
                            <label className="text-xs font-black text-muted-foreground uppercase tracking-widest">對象姓名 *</label>
                            <input
                                type="text"
                                value={formData.name}
                                onChange={e => setFormData({ ...formData, name: e.target.value })}
                                className="w-full bg-muted border-black/30 rounded-xl px-4 py-3 text-sm focus:bg-surface transition-all outline-none border"
                                placeholder="例如：王小明"
                            />
                        </div>
                        <div className="space-y-2">
                            <label className="text-xs font-black text-muted-foreground uppercase tracking-widest">公司全銜 *</label>
                            <input
                                type="text"
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-black text-muted-foreground uppercase tracking-widest">統一編號</label>
                            <input
                                type="text"
                                value={formData.taxId}
                                onChange={e => setFormData({ ...formData, taxId: e.target.value })}
                                className="w-full bg-muted border-black/30 rounded-xl px-4 py-3 text-sm focus:bg-surface transition-all outline-none border font-mono"
                                placeholder="8 位數字"
                            />
                        </div>
                        <div className="space-y-2">
                            <label className="text-xs font-black text-muted-foreground uppercase tracking-widest">電子郵件</label>
                            <input
                                type="email"
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-black text-muted-foreground uppercase tracking-widest">聯絡電話</label>
                            <input
                                type="text"
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-black text-muted-foreground uppercase tracking-widest">通訊地址</label>
                            <input
                                type="text"
                    ...
                    <div className="mt-8 flex justify-end gap-3">
                        <button
                            ...
                            className="px-6 py-3 rounded-xl text-sm font-bold text-muted-foreground hover:bg-slate-100 transition-all"
                        >
                            取消
                        </button>
                        <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCustomers.length === 0 && !isAdding && (
                    <div className="col-span-full py-20 bg-muted rounded-3xl border-2 border-dashed border-black/30 flex flex-col items-center justify-center text-muted-foreground">
                ...
                {filteredCustomers.map(customer => (
                    <div key={customer.id} className="group bg-surface rounded-2xl border border-black/30 p-6 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1.5 h-full bg-slate-100 group-hover:bg-indigo-500 transition-all" />

                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h4 className="text-lg font-black text-foreground leading-tight">{customer.company}</h4>
                                <p className="text-sm font-bold text-muted-foreground mt-1 flex items-center">
                                    <Users className="w-3 h-3 mr-1.5" />
                                    {customer.name}
                                </p>
                            </div>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => startEditing(customer)} className="p-2 hover:bg-slate-100 rounded-lg text-muted-foreground hover:text-indigo-600 transition-all" title="編輯">
                                    <Edit2 className="w-4 h-4" />
                                </button>
                                <button onClick={() => deleteCustomer(customer.id)} className="p-2 hover:bg-rose-50 rounded-lg text-muted-foreground hover:text-rose-600 transition-all" title="刪除">
                        ...
                        <div className="space-y-3 mt-6">
                            {customer.taxId && (
                                <div className="flex items-center text-xs text-foreground font-mono bg-muted p-2 rounded-lg">
                            ...
                            {customer.address && (
                                <div className="flex items-start text-xs text-foreground">
                                    <MapPin className="w-3.5 h-3.5 mr-2 mt-0.5 text-muted-foreground shrink-0" />
