...
import { VisualsGallery } from '@/components/dashboard/VisualsGallery';
...
import { MessageSquare, Layout, Activity, Sparkles, FileText, CheckCircle2, Sliders, Play, Settings, RefreshCw, Box, AlertCircle, Calendar, DollarSign, Clock, Hash, MapPin, Building, Globe, Mail, Phone, Book, GraduationCap, X, Briefcase, Wand2, Star, Shield, ArrowRight, Download, Link2, Share2, Printer, Search, Users, ShieldCheck, UserCircle2, Workflow, Save } from 'lucide-react';
...
function Dashboard() {
    ...
    const [activeTab, setActiveTab] = useState<'setup' | 'report' | 'chat' | 'quote' | 'plan' | 'execution' | 'team' | 'documents' | 'crm' | 'visuals'>('setup');
                        ...
                        <div className="w-full h-full flex flex-col overflow-hidden">
                            <div className="w-full bg-surface border-b border-border/20 dark:border-transparent shadow-[0_10px_30px_-10px_rgba(0,0,0,0.08)] relative z-40 print:hidden">
                                ...
                                <nav className="flex space-x-2 w-full max-w-[1450px] justify-center whitespace-nowrap overflow-x-auto no-scrollbar" aria-label="Tabs">
                                    <TabButton isActive={activeTab === 'setup'} onClick={() => setActiveTab('setup')} icon={<Sliders className="w-5 h-5 lg:mr-2" />} label="1. 專案設定" activeClass="bg-gradient-to-br from-cyan-400 via-cyan-500 to-emerald-500 text-white shadow-[0_15px_40px_-10px_rgba(6,182,212,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    <TabButton isActive={activeTab === 'report'} onClick={() => setActiveTab('report')} icon={<FileText className="w-5 h-5 lg:mr-2" />} label="2. 分析報告" activeClass="bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-[0_15px_40px_-10px_rgba(16,185,129,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    <TabButton isActive={activeTab === 'plan'} onClick={() => setActiveTab('plan')} icon={<Workflow className="w-5 h-5 lg:mr-2" />} label="3. 執行企劃" activeClass="bg-gradient-to-br from-violet-600 to-purple-700 text-white shadow-[0_15px_40px_-10px_rgba(124,58,237,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    <TabButton isActive={activeTab === 'documents'} onClick={() => setActiveTab('documents')} icon={<ShieldCheck className="w-5 h-5 lg:mr-2" />} label="4. 合約文件" activeClass="bg-gradient-to-br from-rose-500 to-pink-600 text-white shadow-[0_15px_40px_-10px_rgba(244,63,94,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    <TabButton isActive={activeTab === 'crm'} onClick={() => setActiveTab('crm')} icon={<MessageSquare className="w-5 h-5 lg:mr-2" />} label="5. 客戶溝通" activeClass="bg-gradient-to-br from-amber-500 to-orange-600 text-white shadow-[0_15px_40px_-10px_rgba(245,158,11,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    <TabButton isActive={activeTab === 'visuals'} onClick={() => setActiveTab('visuals')} icon={<Wand2 className="w-5 h-5 lg:mr-2" />} label="6. 視覺生成" activeClass="bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-[0_15px_40px_-10px_rgba(99,102,241,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    <TabButton isActive={activeTab === 'quote'} onClick={() => { if (userTier === 'free') setUpgradeModalOpen(true); else setActiveTab('quote'); }} icon={<FileText className="w-5 h-5 lg:mr-2" />} label="7. 報價單" isLocked={userTier === 'free'} activeClass="bg-gradient-to-br from-cyan-400 via-cyan-500 to-emerald-500 text-white shadow-[0_15px_40px_-10px_rgba(6,182,212,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    <TabButton isActive={activeTab === 'execution'} onClick={() => { if (userTier === 'free') setUpgradeModalOpen(true); else setActiveTab('execution'); }} icon={<Activity className="w-5 h-5 lg:mr-2" />} label="8. 財務與執行" isLocked={userTier === 'free'} activeClass="bg-gradient-to-br from-orange-500 to-amber-600 text-white shadow-[0_15px_40px_-10px_rgba(245,158,11,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    {['owner', 'admin'].includes(currentTeamRole) && (
                                        <TabButton isActive={activeTab === 'team'} onClick={() => { if (userTier === 'free') setUpgradeModalOpen(true); else setActiveTab('team'); }} icon={<Users className="w-5 h-5 lg:mr-2" />} label="9. 團隊管理" isLocked={userTier === 'free'} activeClass="bg-gradient-to-br from-slate-700 to-slate-900 text-white shadow-[0_15px_40px_-10px_rgba(30,41,59,0.4)] ring-1 ring-white/40 border-t border-white/30" />
                                    )}
                                </nav>
                            </div>
                        </div>
